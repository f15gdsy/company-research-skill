...
从用户输入中提取公司名称和股票代码（如有）。如果用户只给了公司名没有代码，用 neodata-financial-search 或网络搜索确认代码。
...
**基础信息**：
- 公司全称、公司代码、所在行业、成立时间、总部
- 核心产品/服务是什么
- 上市交易所和代码（如已上市）

**财务数据**（通过 neodata-financial-search 或网络搜索获取）：
- 最近财年收入、净利润、增长率
- 毛利率、净利率、ROE
- 收入按业务线/地区的拆分
...
**为什么必须做这一步**：很多公司的"优势"其实是行业β（行业红利），不是α（公司能力）。不理解行业特点，容易把"风口上的猪"当成"会飞的鹰"。

**提示词思路**：
```
假设我要研究 [公司名]，在深入分析它的护城河之前，先帮我搞清楚它所在的 [行业名] 有什么特点？

请覆盖以下维度：
1. **行业生命周期**：萌芽 / 成长 / 成熟 / 衰退？关键判断依据是什么？
2. **竞争格局**：是分散（CR5 < 30%）还是集中（CR5 > 60%）？[公司名] 在格局中处于什么位置？
3. **盈利能力天花板**：行业平均 ROE / 净利率大概在什么水平？是暴利、薄利、还是周期暴利？
4. **进入壁垒**：新玩家容易进来吗？壁垒是技术、牌照、规模、还是网络效应？
5. **政策敏感度**：这个行业是政策驱动、政策中性、还是政策风险高？
6. **技术迭代速度**：是慢变行业（如公用事业）还是快变行业（如消费电子）？

——追问：以上哪个维度对 [公司名] 的估值影响最大？
```

**期望产出**：一张行业特征卡片（表格形式），标注每个维度的判断依据和证据等级。

**与后续动作的衔接**：
- 如果行业处于"成长 → 成熟"拐点，护城河分析必须追问"增长红利还能吃几年"
- 如果行业集中度低（CR5 < 30%），护城河可能不是"守住"，而是"跑马圈地"
- 如果行业政策敏感度高，风险清单必须加入"政策转向风险"

---

**阶段一小结**：分析完前三个动作后，必须用一段话总结——"搞了半天，它其实是一家______公司，所处的行业正处于 ______ 阶段，这意味着 ______"。这比任何数据都重要。
...
6. **行业β vs 公司α**：它的增长有多少是行业红利，多少是抢份额？如果行业增速放缓，它的护城河还能守住吗？
...
6. **行业性风险**：整个行业都在 downturn，它抗不抗得住？行业性风险 vs 公司特异性风险，哪个更致命？
...
□ 行业特点是否已扫描？行业生命周期/竞争格局/政策敏感度是否已标注？
...
> - 【通用】行业特点扫描——所有行业（行业生命周期/竞争格局/政策敏感度）
...
`neodata-financial-search` 是本 skill 的**首选数据源**，通过自然语言查询即可获取七大类金融数据：

```
@skill:neodata-financial-search 查询 [公司名] 的最新财报和行情
```

| 需求 | 查询方式 |
|------|---------|
| 公司基础信息 + 股票代码 | 自然语言查询公司名，自动识别标的 |
| 实时行情（股价/市值/PE/PB） | 查询"XX 最新行情" |
| 财务报表（营收/净利/ROE/增速） | 查询"XX 最近财年的营收、净利润、毛利率" |
| 收入按业务线拆分 | 查询"XX 的收入结构，各业务线占比" |
| 分析师评级/研报 | 查询"机构对 XX 的最新评级和目标价"（通过文档召回） |
| 板块/行业信息 | 查询"XX 行业板块龙头股估值" |
| 公告/大事件 | 自动包含在查询结果中（`stock_big_event`） |
| 资金流向 | 查询"XX 近期资金流向" |

> `neodata-financial-search` 覆盖股票（A股/港股/美股）、指数、板块、公募基金、宏观经济、外汇、大宗商品七大类，同时支持结构化数据（`apiData`）和文档召回（`docData`，含财经资讯、券商研报、公司公告），是比 Longbridge MCP 更全面的金融数据入口。 |

### 数据源降级顺序

当 neodata-financial-search 不可用或限流时，按以下顺序尝试降级（自动切换，不反复重试同一工具）：

1. **`neodata-financial-search`** — 首选金融数据源。自然语言查询，覆盖七大类金融数据 + 文档召回（研报/公告/资讯）。设置 `data-type: "api"` 获取结构化数据，`data-type: "all"` 同时获取文章。
2. **`mcp__tavily__tavily_search`** — 通用搜索首选。支持：
...
3. **`mcp__tavily__tavily_extract`** — 当已有具体 URL 时（如招股书、财报、百科），直接提取全文内容。设置 `extract_depth: "advanced"`, `format: "markdown"`
4. **`WebSearch`（内置）** — 中英文通用搜索备用
5. **`WebFetch`** — 从已知 URL 提取内容的兜底方案
