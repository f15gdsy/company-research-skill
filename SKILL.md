...
用 3 小时（可压缩到 30 分钟快速版）把一家陌生公司拆透。核心不是堆数据，而是**用对的问题把用户推到独立判断的阶段**。
...
| **B 级** | 行业权威机构 | LightCounting、TrendForce、IDC 市占率数据 | 高 | 高概率判断 |
...
- B 级证据 → 可以用"预计""大概率""处于" 
- C 级证据 → 只能用"估计""约""可能"等倾向性表述
- D 级证据 → 必须加"据产业链消息""市场认为"等限定
- E 级证据 → 必须明确标注"以下为 AI 推理/猜测"
...
□ 不同时期数据的口径是否一致？
    → 例如"2025 全年"用的归母口径，"2026Q1"用的扣非口径→直接对比会误导。
      跨期对比时必须确认分子分母口径一致，不一致的须标注差异。
...
如果用户时间有限或只需要快速概览，可以压缩为 30 分钟快速版：

1. **类比 + 业务版图**（10 分钟）：只抓最大的收入线和最反直觉的发现 → **一句话小结"它其实是一家______公司"**
2. **找命脉**（10 分钟）：只问"真正的护城河是什么，哪些是假的" → **一句话小结"最核心的原因是______"**
3. **反方辩手 + 判断**（10 分钟）：只问"最大的三个风险是什么"

每个阶段保留一句话总结，不可省略。笔记在开头标注 `(快速版)`。
